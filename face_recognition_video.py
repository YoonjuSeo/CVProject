...
def detectAndDisplay(image):
    ...
    #Show the output image, show in half size
    image = cv2.resize(image, None, fx=0.5, fy=0.5)
    cv2.imshow("Recognition", image)

    # If the video wirter is None *AND* we are supposed to write the output video to disk initialize the writer
    global writer
    if writer is None and output_name is not None:
        fourcc = cv2.VideoWriter_fourcc(*"MJPG")
        writer = cv2.VideoWriter(output_name, fourcc, 25, (image.shape[1], image.shape[0]), True)
